(function() {
  ...
  angular
    .module('cpo')
    .service('CustomerMasterService', ['$http','$timeout', '$translate', 'CommonService', '$uibModal', 'uiGridConstants', 'uiGridGroupingConstants',
      function($http, $timeout,$translate, CommonService, $uibModal, uiGridConstants, uiGridGroupingConstants) {
        ...
        this.getList = function(scope) {
          ...
          if(scope.searchCustomerNo&&scope.searchCustomerNo.length>0){
              param['like_customerNo'] = scope.searchCustomerNo;
          }
          ...
          GLOBAL_Http($http, "cpo/api/worktable/customerinfo/find?", 'GET', param, function(data) {
        ...
        this.init = function(scope) {
         ...
         var staticColumns =  CommonService.constructeStaticColumnsFromJSON(scope,"customerMaster",false,null,180);
    ...
    .controller('customerMasterCtrl', ['$scope', 'CustomerMasterService',
      function($scope, CustomerMasterService) {

        $scope.getList = function() {
          CustomerMasterService.getList($scope);
        }

        $scope.importFile = function() {
          CustomerMasterService.importFile($scope);
        }
        $scope.toggleFilterRow = function() {
          CustomerMasterService.toggleFilterRow($scope);
        };
        $scope.exportFile = function () {
          CustomerMasterService.exportFile($scope);
        }
        $scope.search = function () {

          CustomerMasterService.search($scope);
        }

        CustomerMasterService.init($scope);
      }
    ])
})();
