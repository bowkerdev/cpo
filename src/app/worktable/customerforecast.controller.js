    ...
    .service('CustomerForecastService', ["$timeout", '$http', '$translate', 'CommonService', '$uibModal', 'uiGridConstants', 'uiGridGroupingConstants', 'factoryCapacityService','workTableCommonService',
      function ($timeout, $http, $translate, CommonService, $uibModal, uiGridConstants, uiGridGroupingConstants, factoryCapacityService,workTableCommonService) {
            ...
            onRegisterApi: function (gridApi) {
              ...
              gridApi.pagination.on.paginationChanged(scope, function (newPage, pageSize) {
                ...
                switch (i) {
                  ...
                  case 7: {
                    _this.getTransitOrder(scope,scope.page7,4,false);
                    break;
                  }
                  case 8: {
                    _this.getTransitOrder(scope,scope.page8,5,false);
                    break;
                  }
                }
              });
              ...
              gridApi.core.on.filterChanged(scope, function (col) {
                ...
                scope.filterTimeout = $timeout(function () {
                  ...
                  if (i == 2) {
                    ...
                    _this.getTransitOrder(scope,scope.page8,5,true);
                  }
        ...
        this.getAllHistoryOrder = function(scope){
          var _this = this;
          GLOBAL_Http($http, "/cpo/api/document/query_order_document?", 'GET', { orderType: "2"}, function(data) {
              ...
              scope.documentTypes = data.output.map(function(item,index){
                return {
                  ...
                  documents:item.documents.map(function(item2,index2){
                    return {id:item2.documentId,label:item2.documentOldName+" ("+ new Date(item2.utcCreate).toLocaleDateString()+")"}
                  })
                }
              });
        ...
        this.refreshAll = function (scope) {
          ...
          this.getTransitOrder(scope,scope.page7,4,true);
          this.getTransitOrder(scope,scope.page8,5,true);
        ...
        this.getSeasonList = function(scope) {
          ...
          GLOBAL_Http($http, "cpo/api/worktable/query_season?", 'GET', {}, function(data) {
            if(data.output) {
              scope.seasonList =data.output.map(function(item){return {id:item.label,label:item.label}})
            ...
            if(data.status == 0) {
              if(data.output) {
                if(status == '4') {
                  ...
                  scope.navList[5].count = data.totalCount? data.totalCount:"0";

                  if(scope.selectDocumentType.id=='11'){
                    scope.gridOptions7.columnDefs =  workTableCommonService.constructeAssignmentStaticColumns(scope,"customerforecast",true,150);
                    workTableCommonService.cusorderDynamicColumnsForAChinaBuyPlan(data.sizeListCount , scope.gridOptions7);
                  ...
                  scope.navList[6].count = data.totalCount? data.totalCount:"0";

                  if(scope.selectDocumentType.id=='11'){
                    scope.gridOptions8.columnDefs =  workTableCommonService.constructeAssignmentStaticColumns(scope,"customerforecast",true,150);
                    workTableCommonService.cusorderDynamicColumnsForAChinaBuyPlan(data.sizeListCount , scope.gridOptions8);
        ...
        this.getAssignFactoryResult = function (scope, type, status, page, shouldPageNumberReset) {
          ...
          GLOBAL_Http($http, "cpo/api/worktable/query_assignment_result?", 'POST', param, function (data) {
            ...
            if (data.output) {
              ...
              switch (status) {
                case '0,3': {
                  scope.customerForecastNewPending = translateData(data.output);
                  scope.gridOptions2.data = scope.customerForecastNewPending;
                  ...
                  if(scope.selectDocumentType.id=='11'){
                    scope.gridOptions2.columnDefs =  workTableCommonService.constructeAssignmentStaticColumns(scope,"customerforecast",true,150);
                    workTableCommonService.cusorderDynamicColumnsForAChinaBuyPlan(data.sizeListCount , scope.gridOptions2);
                ...
                case 2: {
                  scope.customerForecastNewOrder = translateData(data.output);
                  scope.navList[1].count = data.totalCount? data.totalCount:"0";
                  scope.gridOptions3.data = scope.customerForecastNewOrder;
                  scope.page3.totalNum = data.total;
                  scope.gridOptions3.totalItems = scope.page3.totalNum;
                  scope.tabStatus.tabIndex2 = true;

                  if(scope.selectDocumentType.id=='11'){
                  scope.gridOptions3.columnDefs =  workTableCommonService.constructeAssignmentStaticColumns(scope,"customerforecast",true,150);
                  workTableCommonService.cusorderDynamicColumnsForAChinaBuyPlan(data.sizeListCount , scope.gridOptions3);
        ...
        this.init = function (scope) {
          ...
          [{name:"New Pending",count:0},
            {name:"New Order",count:0},
            {name:"SLT Working No.",count:0},
            {name:"SLT Summary Report",count:0},
            {name:"SLT Summary Report By Working No.",count:0},
          ...
          scope.customerForecastNewPending = [];
          scope.customerForecastNewOrder = [];
          //					this.initData(scope);
          ...
          this.initGripOptionTwo(scope, 2, 'customerForecastNewPengding');
          ...
          scope.gridOptions2.data = scope.customerForecastNewPending;
          scope.gridOptions2.data = scope.customerForecastNewOrder;
          for (var i = 2; i < 3; i++) {
            if (i == 4) continue;
              ...
              var modalInstance =  $uibModal.open({
                ...
                controller: function($scope,$uibModalInstance) {
                  ...
                  $scope.submit = function () {
                    ...
                    $uibModalInstance.resolve({season:$scope.selectSeason.id});
    ...
    .controller('CustomerForecastCtrl', ['$scope', 'CustomerForecastService','FilterInGridService',
      function ($scope, CustomerForecastService,FilterInGridService) {
        ...
        $scope.editFactoryCapacity = function () {
          CustomerForecastService.editFactoryCapacity($scope);
        }
