			...
			function($http, $translate, CommonService, $uibModal) {
				...
				this.fetchTabInfo = function(scope) {
					...
					if(scope.tabindex == 1) {
						...
						this.getData(scope, 'cpo/api/worktable/potransferExt/query_po_transfer?', 'isPoTransfer')
				...
				this.getTitle = function(scope, list) {
					...
					for(var i = 0; i < list.length; i++) {
						columnDefs.push({
							name: list[i].jsonObjectKey,
							displayName: list[i].headerName,
							field: list[i].jsonObjectKey,
							minWidth: '120',
				...
				this.getData = function(scope, url) {
					...
					if(scope.tabindex == 1) {
						param['code'] = 'po_lo_report';
					} else if(scope.tabindex == 2) {
						param['code'] = 'po_internal_report';
					}
					var staticColumns;
					GLOBAL_Http($http, url, 'GET', param, function(data) {
						if(scope.tabindex == 1 || scope.tabindex == 2) {
							if(data.output && data.output.rows) {
								scope.page.totalNum = data.output.total;
								scope.gridOptions.totalItems = scope.page.totalNum;
								var result = translateData(data.output.rows);
								scope.customerReportOne = result;
								scope.gridOptions.columnDefs = _this.getTitle(scope, data.jsonExportEntries);
							} else {
								scope.customerReportOne = [];
							}
						} else {
